...
<#assign makeCatalogType = ["片条加工","打胎工艺","花丝工艺","实镶工艺","金银蜡工艺","錾刻工艺","铆接工艺","焊接工艺","烧蓝工艺","鎏金工艺","表面处理工艺","工艺视频"] />
...
<div class="details">
	<table width="100%">
		...
		<tr> 
			<td  colspan="4">
				<font color="red">注意:1、工艺描述相当于该工艺备注,便于后台维护和查看,无实际意义</font>
				<br>
			</td>
		</tr>
		<tr>
			...
			<td align="right" width="150">工艺展示分类:</td>
			<td>
			<#--<input type="text" id="makeProductseq" name="makeProductseq" class="field-required" <#if makes?has_content><#if makes.MAKEPRODUCTSEQ?exists> value="${makes.MAKEPRODUCTSEQ!}"</#if></#if>/>--> 
				<select id="makeproductcatalog" class="form-builder-field-node" name="makeproductcatalog">
					<#list makeCatalogType as maketype>
						<option value="${maketype_index}" <#if makes?has_content><#if makes.MAKEPRODUCTCATALOG?exists><#if makes.MAKEPRODUCTCATALOG=='${maketype_index}'>selected="true"</#if></#if></#if>>${maketype}</option>
					</#list>
				</select>
			</td>
		</tr>
		...
		if(hascontent==true){
			...
			$.ajax({
				...
				data:{
					...
					makeproductcatalog:$("#makeproductcatalog").val(),
					content:content,
