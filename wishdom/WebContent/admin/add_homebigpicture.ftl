...
<script type="text/javascript">
    ...
    ue.addListener("ready", function () {
        ue.focus(true);
        <#if homebigpicture?has_content>
        	UE.getEditor('editor').setContent('${homebigpicture.CONTENT}');
        </#if>
    });
    function setContent() {
        <#if homebigpicture?has_content>
	...
	function submit(){
		...
		if(hascontent==true){
			...
			$.post('${contextPath}/admin/update_homebigpicture.htm',{content:content},function(data){if(data=='success'){alert("保存成功");}else{alert("保存失败");}});
			return;
		}else{
			alert("请先国匠文化内容");
			return;
