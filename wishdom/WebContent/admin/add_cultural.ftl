...
<script type="text/javascript">
    ...
    ue.addListener("ready", function () {
        ue.focus(true);
        <#if cultural?has_content>
        	UE.getEditor('editor').setContent('${cultural.CONTENT}');
        </#if>
    });
    function setContent() {
        <#if cultural?has_content>
	...
	function submit(){
		...
		if(hascontent==true){
			...
			$.post('${contextPath}/admin/update_cultural.htm',{content:content},function(data){if(data=='success'){alert("保存成功");}else{alert("保存失败");}});
			return;
		}else{
			alert("请先国匠文化内容");
			return;
