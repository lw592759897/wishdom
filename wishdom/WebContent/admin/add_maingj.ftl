...
<div class="details">
	<table width="100%">
		<tr align="center">
			<td>
				<b style="font-size:16px">编辑首页关于国匠</b>
				<hr>
			</td>
		</tr>
		<tr>
...
<script type="text/javascript">
    ...
    ue.addListener("ready", function () {
        ue.focus(true);
        <#if maingjcontent?has_content>
        	UE.getEditor('editor').setContent('${maingjcontent.CONTENT}');
        </#if>
    });
    function setContent() {
        <#if maingjcontent?has_content>
	...
	function submit(){
		...
		if(hascontent==true){
			...
			$.post('${contextPath}/admin/update_maingj.htm',{content:content},function(data){if(data=='success'){alert("保存成功");}else{alert("保存失败");}});
			return;
		}else{
			alert("请先填写首页关于国匠内容");
			return;
